 ...
 * @namespace
 * @name pkg_vis
 * @description GEE Visualization package
 */
var pkgs = {};
 ...
 * pkgs.series(imgcol, {}, 'bandname', region, label);
 * @memberof pkg_vis
 */
pkgs.series = function(ImgCol, vis, name, region, label, scale) {
    scale   = scale || 2000;
 ...
 * @example
 * var pkgs   = require('users/kongdd/public:pkgs.js');
 * var vis_vi   = {min: 0, max: 5000, palette:pkgs.colors.RdYlGn[11]};
 * var vis_diff = {min: 0, max: 1000, palette:pkgs.colors.RdBu[11]};
 * var lg_vi   = pkgs.grad_legend(vis_vi  , 'VI', false), 
 *     lg_diff = pkgs.grad_legend(vis_diff, 'diff', false); 
 * pkgs.add_lgds([lg_vi, lg_diff]);
 * @memberof pkg_vis
 */
pkgs.add_lgds = function(lgds, map) {
    map = map || Map;
    lgds = ui.Panel({
        widgets: lgds,
...
/** 
 * Gradient legend in GEE 
 */
pkgs.legend = function (viz, title, IsPlot, position, fontSize) {
...
/** DEPRECATED */
pkgs.legend_vert = pkgs.legend;
pkgs.legend_grad = pkgs.legend;
pkgs.grad_legend = pkgs.legend_grad;

/**
 * Creates a color bar thumbnail image for use in legend from the given color 
 * palette.
 */
pkgs.makeColorBarParams = function (palette) {
    return {
        bbox: [0, 0, 1, 0.1],
        dimensions: '100x10',
        format: 'png',
...
// horz: horizontal
// vert: vertical
pkgs.legend_horz = function (vis, title, IsPlot, position, fontSize) {
    fontSize = fontSize || "20px";
    title = title || "";
    ...
    var colorBar = ui.Thumbnail({
        ...
        params: pkgs.makeColorBarParams(vis.palette),
    ...
    var legendTitle = ui.Label({
        value: title, style: { fontWeight: 'bold' }
    });
 ...
 * @example
 * var pkgs   = require('users/kongdd/public:pkgs.js');
 * var vis = { bands: ['occurrence'], min: 0.0, max: 100.0, palette: ['ffffff', 'ffbbbb', '0000ff'] };
 * pkgs.legend_grad(vis, 'Water Occurrence');
 * 
 * var vis_et = {min:0, max:3000, palette: pkgs.colors.RdBu[10]};
 * pkgs.legend_discrete(vis_et, 'ET_pot mm/y');
 * @memberof pkg_vis
 */
pkgs.legend_discrete = function (names, palette, title, IsPlot, position, fontSize) {
    fontSize = fontSize || "20px";
    title = title || "";
    position = position || "bottom-left";
    if (IsPlot === undefined) IsPlot = true;
...
/** DEPRECATED */
pkgs.discrete_legend = pkgs.legend_discrete;
 ...
 * @memberof pkg_vis
 */
pkgs.layout = function(nmap, nrow, ncol, byrow) {
    nmap = nmap || 2;
 ...
 * };
 * pkgs.layout2(options)
 * @memberof pkg_vis
 */
pkgs.layout2 = function(options) {
 ...
 */
pkgs.show_annual_imgcol = function(maps, imgcol, options) {
    ...
    if (vis) {
        ...
        var lgd = pkgs.grad_legend(vis, band, false); //gC m-2 y-2
        maps[0].add(lgd);
    }
...
var debug = false;
if (debug) {
    // pkgs   = require('users/kongdd/public:pkgs.js');
    ...
    var maps = [];
    pkgs.show_annual_imgcol(maps, imgcol, options);
}
