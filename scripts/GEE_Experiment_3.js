...
// actual evapotranspiration model – comment on “Monitoring 
...
/* GEE Experiment 3 Javascript demonstration:

  A comparison of mean monthly Landsat 8 CMRSET’s 
  (Guerschman et al., 2009, https://doi.org/10.1016/j.jhydrol.2009.02.013) 
  irrigation estimation (Irrest) obtained using SWIR1 and SWIR2 in the computation of GVMI. 
  The hypothesis is that the crop factor kc computed with SWIR2 is higher than with SWIR1, 
  therefore ETa and Irrest are overestimated, supporting that 
  Bretreger et al.'s (2020, https://doi.org/10.1016/j.jhydrol.2020.125356) 
  implementation of CMRSET is flawed. 
  For ease of access, this experiment uses Landsat 8 surface reflectance data and potential 
  evapotranspiration and rainfall data from ERA5-Land Monthly Averaged - ECMWF Climate Reanalysis,
  both available through Google Earth Engine.
...
function evapkamble(image){
  ...
  // Actual evapotranspiration parameters
  // In ERA (mm/d) negative values indicate evaporation hence multiply by -1
...
var Irrest_SWIR12 = Irrestdiff.select(["Irrest_Irrisat", "Irrest_Kamble", "IrrestSWIR2","IrrestSWIR1"]);
...
var chart =
    ui.Chart.image
        ...
        .setSeriesNames(["Irrest CMRSET with Landsat SWIR1", "Irrest CMRSET with Landsat SWIR2","Irrest Irrisat" ,"Irrest Kamble" ])
        .setOptions({
