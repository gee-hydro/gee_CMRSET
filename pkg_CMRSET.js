...
function gvmi(image) {
    // Global vegetation moisture index parameters Nill, Equation (3)
...
var LS_masked = LS_collection.map(maskClouds).map(ScoreMask); // Cloud masking
...
var innerJoin = ee.ImageCollection(simpleJoin.apply(Kc, ERA5, filter)); // Inner join
